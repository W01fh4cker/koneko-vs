import re

def convert_shellcode(shellcode_c):
    # 提取所有十六进制字节值
    hex_bytes = re.findall(r'0x([0-9a-fA-F]{2})', shellcode_c, re.IGNORECASE)
    
    # 将十六进制值转换为字节数组
    byte_data = bytes.fromhex(''.join(hex_bytes))
    
    # 格式化为Python字节字符串（每行12字节）
    formatted = []
    line_chunk = 12
    for i in range(0, len(byte_data), line_chunk):
        chunk = byte_data[i:i+line_chunk]
        hex_str = ''.join([f'\\x{b:02x}' for b in chunk])
        formatted.append(f'buf += b"{hex_str}"')
    
    return '\n'.join(formatted)

# 替换成用户提供的完整未截断shellcode
real_shellcode = r"""
    unsigned char shellcode[] = {  };
"""

# 运行转换
output = convert_shellcode(real_shellcode)

# 打印结果
print("完整Python格式化代码：\n")
print("buf =  b\"\"")
print(output)